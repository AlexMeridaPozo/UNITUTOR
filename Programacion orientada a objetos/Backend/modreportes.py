...
def load_data_variable(path, variable_name):
...
def load_estudiantes():
    return load_data_variable(ESTUDIANTES_FILE, "ESTUDIANTES")


def load_tutores():
    return load_data_variable(TUTORES_FILE, "TUTORES")


def load_tutorias():
    return load_data_variable(TUTORIAS_FILE, "TUTORIAS")
...
class MiPanelReportes(wx.Panel):
    def __init__(self, parent):
        ...
        # Cargamos datos desde los archivos en Datos
        tutores = load_tutores()
        estudiantes = load_estudiantes()
        tutorias = load_tutorias()

        def _compute_counts():
            tutores_local = load_tutores()
            estudiantes_local = load_estudiantes()
            tutorias_local = load_tutorias()
        ...
        num_tutores, num_estudiantes, num_tutorias_programadas, num_tutorias_completadas = _compute_counts()
    ...
    def actualizar_conteos(self):
        ...
        num_tutores = sum(1 for t in load_tutores() if len(t) > 4 and str(t[4]).strip().lower() == "activo")
        num_estudiantes = sum(1 for e in load_estudiantes() if len(e) > 4 and str(e[4]).strip().lower() == "activo")
        num_tutorias_programadas = sum(1 for t in load_tutorias() if len(t) > 4 and str(t[4]).strip().lower() in ("programada", "pendiente"))
        num_tutorias_completadas = sum(1 for t in load_tutorias() if len(t) > 4 and str(t[4]).strip().lower() in ("realizada", "completada", "completado"))
...
class ReportesFrame(wx.Frame):
    ...
    def exportar_PDF(self, evt):
        ...
        bloques +=  self.tabla_de("Tutores", load_tutores(),
                                   ["Legajo", "Nombre", "Materia", "Disponibilidad", "Estado"], estilos)
        bloques +=  self.tabla_de("Estudiantes", load_estudiantes(),
                                   ["Legajo", "Nombre", "Carrera", "Año", "Estado"], estilos)
        bloques +=  self.tabla_de("Tutorias", load_tutorias(),
