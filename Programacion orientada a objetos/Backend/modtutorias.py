...
# ----------------------------------------------------------------------
class TutoriasModel(dv.DataViewIndexListModel):
    ...
    def SetValueByRow(self, value, row, col):
        ...
        if col == 4:
            estados_validos = {"Realizada", "Pendiente", "Cancelada"}
            if value not in estados_validos:
                wx.MessageBox(
                    f"Valor de estado '{value}' no se reconoce. Usa 'Realizada', 'Pendiente' o 'Cancelada'.",
    ...
    def GetAttrByRow(self, row, col, attr):
        ...
        if col == 4:
            ...
            if estado == "Realizada":
                ...
                attr.SetColour(wx.Colour(255, 165, 0)) #Naranja
                attr.SetBold(True)
    ...
    def Compare(self, item1, item2, col, ascending):
        ...
        # Compara el número del legajo
        if col in [0, 3]:
            try:
                a_num = int(''.join(filter(str.isdigit, a)))
                b_num = int(''.join(filter(str.isdigit, b)))
                if a_num < b_num: return -1 #si el primero es menor al siguiente legajo le resta 1
                if a_num > b_num: return 1 #si el primero es mayor al siguiente legajo le suma 1
                return 0
            except ValueError:
                pass
    ...
    def DeleteRows(self, rows):
        # para que no haya desfase de indices
...
# ----------------------------------------------------------------------
# PANEL DEL MÓDULO: GESTIÓN DE Tutorias (SCREEN 3 - C)
# ----------------------------------------------------------------------
class TutoriasPanel(wx.Panel):
    def __init__(self, parent):
        super().__init__(parent)

        datos_iniciales = self.CargaTutorDatos()

        self.model = TutoriasModel(datos_iniciales)

        # Sizer Principal del panel
        sizer_efectos = wx.BoxSizer(wx.VERTICAL)

        # Título de la sección
        lbl_titulo = wx.StaticText(self, label="Gestión de Tutorías")
        ...
        # Barra de Búsqueda (Filtro interactivo)
        ...
        # Control de Vista de Datos Avanzado (DataViewCtrl)
        self.dvc = dv.DataViewCtrl(self, style=wx.BORDER_THEME | dv.DV_ROW_LINES | dv.DV_VERT_RULES | dv.DV_MULTIPLE)
        self.dvc.AssociateModel(self.model)

        # Pone las columnas requeridas a tutorias
        self.dvc.AppendTextColumn("Legajo", 0, width=65, mode=dv.DATAVIEW_CELL_INERT)
        self.dvc.AppendTextColumn("Tutor", 1, width=170, mode=dv.DATAVIEW_CELL_EDITABLE)
        self.dvc.AppendTextColumn("Estudiante", 2, width=150, mode=dv.DATAVIEW_CELL_EDITABLE)
        self.dvc.AppendTextColumn("Fecha", 3, width=105, mode=dv.DATAVIEW_CELL_EDITABLE)
        self.dvc.AppendTextColumn("Estado", 4, width=95, mode=dv.DATAVIEW_CELL_EDITABLE)

        # Permite ordenar/reordenar por columnas
        ...
        # Panel Inferior (Botones de acción + Contador Dinámico)
        ...
        # es un espaciador para que ponga el contador a la derecha
    ...
    def ActualizarContador(self):
        """Calcula de la lista maestra en memoria cuántos registros están realizados"""
        # cuenta las tutorías con estado 'Realizada'
        realizadas = len([
            reg for reg in self.model.all_data
            if len(reg) > 4 and str(reg[4]).strip().lower() == "realizada"
        ])
        activos = realizadas
        total = len(self.model.all_data)
        # muestra el número de realizadas para claridad
        self.lbl_contador.SetLabel(f"Realizadas: {activos}  |  Total: {total}")
        self.Layout()

    def CargaTutorDatos(self):
        """Carga los datos de tutores desde un archivo Python local."""
        if os.path.exists(TUTORIAS_FILE):
            namespace = {}
            with open(TUTORIAS_FILE, "r", encoding="utf-8") as f:
                code = f.read()
            exec(code, namespace)
            data = namespace.get("TUTORIAS")
            if isinstance(data, list) and all(isinstance(row, list) for row in data):
                return data

        datos_iniciales = [
            ["T-001", "Garcia, Maria", "Pérez, Lucas", "10/11/25", "Realizada"],
            ["T-002", "Lopez, Juan", "Gomez, Laura", "12/11/25", "Pendiente"],
            ["T-003", "Martinez, Ana", "Rodriguez, Sofia", "15/11/25", "Cancelada"]
        ]
        self.GuardaTutoriasDatos(datos_iniciales)
        return datos_iniciales

    def GuardaTutoriasDatos(self, data=None):
        """Guarda los datos de tutorias en un archivo Python local."""
        data_to_save = self.model.all_data if data is None else data
        with open(TUTORIAS_FILE, "w", encoding="utf-8") as f:
    ...
    def OnAgregarRegistro(self, evt):
        ...
        # Genera una nueva fila editable por defecto
        nueva_tutoria = [nuevo_legajo, "Nuevo Tutor", "Asignar Estudiante", "dd/mm/yy", "Pendiente"]
        self.model.AddRow(nueva_tutoria)
        self.GuardaTutoriasDatos()
        self.ActualizarContador()
...
# ----------------------------------------------------------------------
class TutoriasAppFrame(wx.Frame):
    def __init__(self):
        ...
        for mod in modulos_superiores:
            ...
            btn.SetHoverColour(wx.Colour(155, 200, 230) if mod == "Tutorías" else wx.Colour(220, 220, 220))
            btn.SetFocusColour(wx.Colour(30, 90, 140) if mod == "Tutorías" else wx.Colour(180, 180, 180))

            if mod == "Tutores":
                btn.Bind(wx.EVT_BUTTON, self.AbrirTutores)      
            elif mod == "Estudiantes":                                 #Eventos al apretar los botones respectivos
        ...
        self.panel_contenido = TutoriasPanel(self)
        sizer_ventana.Add(self.panel_contenido, 1, wx.EXPAND)

        # Barra de estado
        self.CreateStatusBar()
        self.SetStatusText("Módulo Activo: Gestión de Tutorías | Datos preparados para futura extracción")
    ...
    def VentanaMenuPrincipal(self, evt=None):
        """Abre la ventana de Estudiantes desde el módulo de Tutorias."""
        self.Hide()
    ...
    def AbrirReportes(self, evt):
        """Abre la ventana de Reportes desde el módulo de Tutorias."""
        self.Hide()
